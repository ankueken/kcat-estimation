...
%% max enzyme activity
load('Result1.mat','EA*','gDW_per_gFW','id*')

EA_Pyl.data_umol_gFW_d = (EA_Pyl.data/1000)*1440; % nmol/gFW/min -> umol/gFW/d
EA_Pyl.data_umol_gDW_d = EA_Pyl.data_umol_gFW_d./repmat(gDW_per_gFW,size(EA_Pyl.data,1),1); % umol/gFW/d -> umol/gDW/d
[EC_fit_Pyl,id_EC,id_data_Pyl]=intersect(EC,EA_Pyl.EC);
vmax_Pyl = nan(length(EC),size(EA_Pyl.data,2));
vmax_Pyl(id_EC,:) = EA_Pyl.data_umol_gDW_d(id_data_Pyl,:);

EA_Piques.data_umol_gFW_d = (EA_Piques.data/1000)*1440; % nmol/gFW/min -> umol/gFW/d
EA_Piques.data_umol_gDW_d = EA_Piques.data_umol_gFW_d/0.088; % umol/gFW/d -> umol/gDW/d
% EA_Piques.data_umol_gFW_d_lb = (EA_Piques.data_lb/1000)*1440; % nmol/gFW/min -> umol/gFW/d
% EA_Piques.data_umol_gDW_d_lb = EA_Piques.data_umol_gFW_d_lb/0.088; % umol/gFW/d -> umol/gDW/d
% EA_Piques.data_umol_gFW_d_ub = (EA_Piques.data_ub/1000)*1440; % nmol/gFW/min -> umol/gFW/d
% EA_Piques.data_umol_gDW_d_ub = EA_Piques.data_umol_gFW_d_ub/0.088; % umol/gFW/d -> umol/gDW/d
[EC_fit_Piques,id_EC,id_data_Piques]=intersect(EC,EA_Piques.EC);
vmax_Piques = nan(length(EC),size(EA_Piques.data,2));
vmax_Piques(id_EC,:) = EA_Piques.data_umol_gDW_d(id_data_Piques,:);

EA_Sulpice.data_umol_gFW_d = (EA_Sulpice.data/1000)*1440; % nmol/gFW/min -> umol/gFW/d
EA_Sulpice.data_umol_gDW_d = EA_Sulpice.data_umol_gFW_d./0.088; % umol/gFW/d -> umol/gDW/d
[EC_fit_Sulpice,id_EC,id_data_Sulpice]=intersect(EC,EA_Sulpice.EC);
vmax_Sulpice = nan(length(EC),size(EA_Sulpice.data,2));
vmax_Sulpice(id_EC,:) = EA_Sulpice.data_umol_gDW_d(id_data_Sulpice,:);

v_max_max=max([vmax_Pyl vmax_Piques vmax_Sulpice],[],2,"omitnan");
id_vmax=find(~isnan(v_max_max));
...
for co = co_ratio
    ...
    for ss=ss_ratio
        ...
        % vmax upper bounds
        model_irr.S = [model_irr.S;
                    Z(id_vmax,:)];
        model_irr.b = [model_irr.b; v_max_max(id_vmax)];
        model_irr.csense = [model_irr.csense; repmat('L',length(id_vmax),1);];
        for i=1:size(model_irr.S,1)
            model_irr.mets{i,1} = strcat('M',num2str(i));
        end
        for i=1:size(model_irr.S,2)
            model_irr.rxns{i,1} = strcat('R',num2str(i));
        end
